# DoD Team Semiclip
Day of Defeat — A *Meta Mod* Team Semiclip
* Allows players pass through other players.
* Based on [[Metamod] Semiclip (v2.2)](https://forums.alliedmods.net/showthread.php?t=250891) by [slLent](https://forums.alliedmods.net/member.php?u=76057) — AlliedModders.
* Based on [ReSemiclip](https://github.com/rehlds/resemiclip) by [s1lentq](https://github.com/s1lentq) and [ReHLDS Team](https://github.com/rehlds) — GitHub.
* Might work on other HL1-based games too.
* Works on `rehlds`, `steam_legacy` and `beta`.
* You can see it or read more details about it @ [AlliedModders](https://forums.alliedmods.net/showthread.php?t=352371).
## Contributors ##
- [s1lentq](https://github.com/s1lentq)
- [Fysiks](https://github.com/fysiks1)
